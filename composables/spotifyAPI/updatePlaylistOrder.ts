...
export const updatePlaylistOrder = async (
  ...
  try {
    ...
    for (const track of reverseSortedTracks) {
      ...
      const rangeLength = 1;
      await updateTrackPositionInPlaylist(
        accessToken,
        playlistId,
        rangeStart,
        insertBefore,
      ...
      filteredTracksOriginalOrder = updateFilteredTracksOriginalOrder(
        filteredTracksOriginalOrder,
        rangeStart,
      );
    }
  } catch {
    nuxtApp.callHook(
      "app:error",
      createError("Could not update playlist's order."),
    );
  }
};
...
const updateFilteredTracksOriginalOrder = (
  array: FilteredTrack[],
  position: number,
) => {
...
const sortTracks = (
  tracksByArtist: { [key: string]: FilteredTrack[] },
